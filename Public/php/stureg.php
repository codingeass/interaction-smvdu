...
if( isset($_REQUEST["name"]) && isset($_REQUEST["email"]) && isset($_REQUEST["pass"]) && isset($_REQUEST["type"]) && isset($_REQUEST["sex"]) && isset($_REQUEST["ent"]) &&isset($_REQUEST["bran"]) && isset($_REQUEST["sem"]) &&isset($_REQUEST["bir"]) &&isset($_REQUEST["join"]) && isset($_REQUEST["add"]) &&isset($_REQUEST["cont"]) &&isset($_REQUEST["pro"]) && isset($_REQUEST["are"])){
	...
	$query="Insert into user (name,email,password,type)values('".urldecode(strtolower(strip_tags($_REQUEST["name"])))."','".urldecode(strip_tags($_REQUEST["email"]))."','".md5(md5(urldecode(strip_tags($_REQUEST["pass"]))))."','".urldecode(strip_tags($_REQUEST["type"]))."');";
	$result=mysql_query($query)
	...
	{                
    $quer="Insert into student (id,name,sex,entry_no,branch,semester,date_of_birth,date_of_joining,address,contact,project,area_of_interest)values('".$res["id"]."','".urldecode(strip_tags($_REQUEST["name"]))."','".strip_tags($_REQUEST["sex"])."','".urldecode(strip_tags($_REQUEST["ent"]))."','".urldecode(strip_tags($_REQUEST["bran"]))."','".urldecode(strip_tags($_REQUEST["sem"]))."','".urldecode(strip_tags($_REQUEST["bir"]))."','".urldecode(strip_tags($_REQUEST["join"]))."','".urldecode(strip_tags($_REQUEST["add"]))."','".urldecode(strip_tags($_REQUEST["cont"]))."','".urldecode(strip_tags($_REQUEST["pro"]))."','".urldecode(strip_tags($_REQUEST["are"]))."');";
